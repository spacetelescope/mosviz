...
root_dir = os.path.join(PACKAGENAME, 'data')
...
if conf.has_section('glue.plugins'):
    glue_plugins = conf.items('glue.plugins')
    for gp in glue_plugins:
        entry_points.setdefault('glue.plugins', []).append(
            '{0} = {1}'.format(gp[0], gp[1]))

# Define entry point for command-line script
entry_points['gui_scripts'] = ['mosviz=mosviz.cli:main']
...
package_info['package_data'][PACKAGENAME].append('controls/slits/*')
